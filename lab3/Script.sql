...
select prod.name,min(ord.snum) from ord inner join prod on ord.pnum = prod.pnum group by prod.name;
--4. Вывести число уникальных продавцов, которые продавали товары покупателям с номерами 2005 и 2004. 
select distinct count(snum) from ord where cnum = 2005 or cnum = 2004;
